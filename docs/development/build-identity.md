...
当前构建身份协议为 5，生成器记录 `embedded-resources: 1`。内嵌资源的路径、大小、原文字节摘要与生成的 C++ 数据共同进入构建身份；产物清单的 `embedded-resources` 与外置 `resources` 分开校验。声明来源纳入隔离快照，编译封存前重扫实际输入，不能在编译期间替换 `dist` 后沿用旧身份。读取产物时也校验清单结构、预算与路径冲突，不执行待检查程序。
