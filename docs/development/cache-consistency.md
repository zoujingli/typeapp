...
PHP 验证使用真实 SQLite 主/副本文件与 Redis，覆盖受控复制延迟、实际写入和回填交错、回滚、TTL、真实 Redis 客户端断连、默认错误与显式回源。两包保持独立，ORM/cache 组合仅在应用示例完成。原生入口为 `docs/build-config/type-cache-consistency.json`。
