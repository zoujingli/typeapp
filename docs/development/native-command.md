...
`scope=pgsql-app` 在独立并发组中只运行 PostgreSQL 应用开发入口，用于定位真实 HTTP 故障，不代表 AOT 或完整平台验收。该诊断将 HTTP 等待设为 15 秒、整套开发检查设为 600 秒，以测量完整请求耗时；报告明确记录诊断标记与预算，默认完整验收不受影响。应用测试保存最近请求与慢请求的耗时；失败时额外记录本轮 PostgreSQL 会话的等待状态和阻塞进程，省略 SQL、请求体和令牌。数据库清理失败时仍保留原请求故障报告，Actions 同时上传脱敏 HTTP 日志。
