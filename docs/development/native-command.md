...
运行扩展与 PHP SDK 的同名动态依赖必须一致。例如 macOS 的 Swoole SQLite hook 与 PHP PDO SQLite 应链接同一份 SQLite 库；混用系统共享映像和另一份同名库，可能在目录包设置加载路径后改变实际解析结果。准备 SDK 时统一依赖并重新构建扩展，随后重建应用、核验实际加载身份和无源码运行；不能删除库摘要或系统映像 UUID 校验来绕过冲突。
