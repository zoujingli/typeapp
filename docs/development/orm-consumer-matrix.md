...
无版本模型的 `increment/decrement` 与版本模型使用相同的事务或保存点保护。`MutationExercise` 通过独立的无版本模型验证：SQLite 末行触发器 `RAISE(FAIL)` 后前面记录不残留修改，三库末行 CHECK 失败完整回滚，保存点失败后外层事务仍可继续，外层回滚覆盖后续成功写入；10001 行完整增减且不影响另一租户。

2026-09-24 在上述回归加入后，macOS ARM64 的三库 PHP 检查通过。含空格路径的独立 Composer 应用复制安装五个生产包，用 TypePHP 0.9.3、PHPX 2.9.2、PHP 8.5.10 ZTS 完整编译 93 项输入；归档并移除消费目录 PHP 源码后，同一产物在不同工作目录通过 MySQL 8.4.11、PostgreSQL 17.11、SQLite 3.53.3 的业务及双进程乐观锁/累加回归。新结果独立记录，不改写下节历史产物身份；其他平台和公开分发子仓精确提交安装未在本次执行。

## 集合写入与组合边界补测

2026-09-24 在 macOS ARM64 上完成本次 ORM 修改的 PHP 三库回归，并在含空格路径的独立 Composer 安装中，用 TypePHP 0.9.3、PHPX 2.9.2 和 PHP 8.5.10 ZTS 编译业务、生成模型及五个生产包的 93 项输入。该消费者同时安装三个驱动，用同一产物验证 SQLite 3.53.3、MySQL 8.4.11、PostgreSQL 17.11。另用原有消费者入口通过 SQLite 单驱动的独立 PHP 安装、平台依赖检查及双进程竞争；两种消费范围分别记录。

归档并逐文件回读校验后移除消费目录中的 2390 个 PHP 文件，从不同工作目录运行原生包。三库均通过超过一万行的集合写入、整条回滚、版本保护、读取组合、关系身份、提交后回调，以及双进程乐观锁和原子累加；累加后最终值为 41、版本为 42。SQLite/PostgreSQL 另验证延迟外键导致的提交未知与对账状态保留，MySQL 对应报告明确标为不支持该故障机制。

产物 SHA-256 为 `244974c6d2459c93fe99d88e6325e20b9bd4a04c6e16e6cc80c64233bf4436d7`，构建身份为 `0c75cea9ba205f6383250ca14c99a294032846481637785caaf0be7218aa7859`。验收源码是 `5a442e2` 基线加本次修改，原始报告保留工作区有修改的事实及逐文件摘要。此记录不覆盖其他平台、公开子仓精确提交消费或全部框架验收；实际运行包仍含原生动态库，不能据此声明完整静态单程序交付已经完成。历史产物继续保留各自身份。
...
每份消费者报告同时保存主仓提交、工作区是否有修改及工具链锁摘要。开发中的成功结果单独成立；当前完整验收覆盖 Linux x64、Linux ARM64、macOS ARM64、Windows x64，只有四个平台均来自同一提交且工作区干净，才计入完整同提交验收。历史三平台矩阵保留原提交和实际范围，不扩写为四平台完成。
