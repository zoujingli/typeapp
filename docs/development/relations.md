...
`load/loadMissing` 只应接收当前模型和数据源的对象列表。当前补加载校验只比较表名与字段名，存在接受另一数据源同形模型的缺陷；调用者不要混合此类对象，框架仍需补齐拒绝检查，见[操作闭环与待闭合项](model-connections.md#操作闭环与待闭合项)。正常预加载通过声明工厂校验数据源，不能据此推定补加载入口已经具备相同检查。
