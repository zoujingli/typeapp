...
Composer生产依赖的`ext-*`要求自动参与探测。构建器读取构建 PHP 中对应扩展声明的必需依赖，递归补齐并按依赖顺序加载；Swoole 运行声明还会显式补齐同一 ABI 的 `curl`，确保 `curl` 在 `swoole` 之前加载。独立消费者默认使用安装后的 `type-build` 内置 Swoole；curl 来自匹配的 SDK、显式 `runtime.modules` 或 CI 的 `TYPE_CURL_MODULE` 候选。需要替换 Swoole 时仍可登记准确文件与摘要。只读扩展元数据不执行应用，不引入可选或冲突扩展。新增依赖的实际版本、模块摘要和加载结果同样进入产物身份。应用额外运行能力使用构建JSON的`runtime`，按真实宿主平台分开：
...
2. 仅为缺失的要求选择共享模块：优先明确的文件/摘要候选。Swoole 随后依次选择有效的 `TYPE_SWOOLE_MODULE`、构建组件自身 `resources/swoole` 中的匹配文件；其他扩展沿用对应 SDK 的标准文件名及既有 curl 环境候选。内置 Swoole 清单缺失、ABI 不匹配、模块摘要或源码适配身份不一致时立即拒绝，不自动下载或回退 SDK 中的 Swoole。
3. 用最终模块组合再次初始化同一embed。启动警告、模块格式/摘要错误、版本/ZTS不符或缺失函数立即拒绝，不等到HTTP服务启动才发现。
4. 收集所选模块及实际动态库依赖，把原始文件、内置模块清单、生成的运行配置、探针源码/二进制与结果纳入构建身份；不会把组件中其他平台的模块声明为应用资源。探测中或指纹收集中发生变化会拒绝。
5. 发布器根据封装清单复制模块和库、生成发布INI，并保留运行扩展/函数门禁。不能在封装后手工补模块或修改摘要来绕过身份。

共享候选的SHA256是调用方固定输入的依据，不独立证明来源真实。模块必须来自可信且匹配的SDK；构建不会自动下载扩展。扩展依赖元数据来自构建 PHP，不能证明另一份候选模块已经可用；候选特有且未声明的依赖、不是正常PHP扩展的库或不兼容ABI，仍由实际加载失败或警告拒绝。构建 PHP 尚未加载的候选若有额外依赖，需在 `runtime.extensions` 中明确补齐。Zend扩展、WASI等不同加载模型没有由本机制自动获得支持。
