...
Composer生产依赖的`ext-*`要求自动参与探测。构建器读取构建 PHP 中对应扩展声明的必需依赖，递归补齐并按依赖顺序加载；例如启用了原生 cURL 的 Swoole 会先加载 `curl`。只读扩展元数据不执行应用，不引入可选或冲突扩展。新增依赖的实际版本、模块摘要和加载结果同样进入产物身份。应用额外运行能力使用构建JSON的`runtime`，按真实宿主平台分开：
