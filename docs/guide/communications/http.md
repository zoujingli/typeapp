...
当前经典 `serve()` 使用 Swoole 的 worker 与信号机制，并显式拒绝 Windows；它没有 `handleSignals` 参数。`stop()` 撤销请求准入、缩短在途期限，worker 内的监督定时器在排空后关闭服务。进程不可用时接入 Swoole 官方线程/协程是统一要求，入口适配仍需完成。
