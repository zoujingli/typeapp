...
在 `app/` 中声明直接继承 `Type\Orm\Model` 的业务类，用 `#[Table(...)]` 指定表，用公开类型属性声明字段。物联中心模型包括平台/客户账号、租户成员、角色权限、产品、物模型、设备、遥测、告警、通知和导出任务；`Column` 补充列名、精确类型、赋值权限和输出可见性，可空性来自 `?string` 等 PHP 类型。

构建器从 `sources` 静态识别模型，保留原类名和业务方法，生成字段映射、水合工厂、查询入口和属性钩子。TypeApp 的身份和物联服务通过显式 Schema/Service 入口访问这些模型，租户条件、角色权限和设备归属必须在查询边界内完成；禁止用通用 users 表或跨租户全表查询替代业务约束。PHP 开发入口先加载本代转换结果，AOT 编译同一结果；业务源码、完整转换结果和生成器身份都进入审计。不要直接加载未经准备的模型源码。
