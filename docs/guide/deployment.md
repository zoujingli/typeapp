...
每个目标平台交付自己的原生程序，外置 `.env` 保存部署配置。PHPX、libphp 及实际使用的原生扩展随程序封装。声明了线程的应用把按本次使用选定的官方 Swoole 链进程序，不再额外携带 `swoole.so`；其余扩展仍随目录包提供。部署人员无需手动安装编译工具链。
...
| 内部运行库 | 程序自行校验和管理；中断或并发首次启动不能使用未完成的载荷 |

一个程序可按入口运行 HTTP、MQTT 或后台角色，角色间通信与进程/线程/协程仍采用 Swoole。程序文件数量与运行角色数量是不同概念，进程不可用时的执行选择见[系统架构](architecture.md#运行方式与平台)。

当前打包工具已提供原生依赖收集、目录包校验与归档，尚未组成可直接执行的单文件程序。单文件释放与启动需验证无源码启动、载荷完整性、并发和中断恢复、含空格目录、配置与数据保留。**下文目录包操作是当前实现，不代表上述交付目标已经完成。**
...
运行包包含原生应用、资源、所需运行库、身份清单、配置示例及操作手册。没有业务 PHP 源码回退，但仍需要匹配的 PHPX、libphp、PDO 等实际原生依赖；不应将其描述为零运行库依赖。线程应用的发布 INI 不再写入 `extension=swoole`，发布目录也不复制 `swoole.so`。
