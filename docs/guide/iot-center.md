...
物联中心是基于 TypeApp 构建的成品案例：一套组合 Plugins、通过 TypePHP 编译、使用 Swoole 原生运行能力的物联网业务应用。它不是 TypeApp 框架本身。框架开发文档在侧栏「TypeApp 框架」和「开发指南」；四者关系见[系统架构](architecture.md)，本页只说明本案例的业务、使用契约和交付边界。

本案例提供两个人员入口：平台管理端面向平台人员，维护账号、租户、站点与全局观察；SaaS 用户端面向客户账号，在当前租户内管理成员、产品、设备、数据与告警。设备经独立 MQTT Broker 接入，遥测由独立接收进程落库，统计、告警、通知和导出分角色运行。MQTT 标准、高可用、目标容量与真实设备仍未完成验收。
...
| 设备接入 | MQTT 5、TLS、QoS 1；独立 Broker 与数据接收；设备凭据、Topic 与归属阶段 | 持久设备路径要求 PostgreSQL 严格同步主备；Windows 上 `iot:mqtt` 会拒绝启动 |
...
全新安装会写入站点默认值：项目名称 `TypeApp`、官网 `https://iots.top`、说明“物联中心管理平台”、默认时区 `Asia/Shanghai`、浅色主题及 Vben 侧栏布局。未登录页面通过 `GET /public/site` 读取公开白名单；平台站点设置页通过 `/admin/site` 读取和保存品牌、主题和布局默认值，使用版本号和审计保护。个人界面偏好由 Vben 原生机制保存在当前浏览器，并按身份域、账号及模拟来源隔离；个人资料页可恢复最新站点默认。官网链接指向项目文档，不会改写本机 API 或设备接入地址。
...
菜单按功能分组，不把所有页面平铺到一级。`web/` 使用 Vben Admin Pro 的布局、权限和表格基础能力，路由进入页面后再加载当前域的数据；组件样式和主题配置应优先复用 Vben 的现有实现。
...
| `iot:mqtt` | 独立 Broker；当前接入策略需要 Unix 非阻塞管道，Windows 启动会拒绝；准备真实 TLS、PostgreSQL 同步主备、唯一节点身份和 worker 命令 |
...
完整配置在 `config/app.php` 与 `.env.example`。应用必须使用匹配的 Swoole 原生扩展；当前配置保留 `IOT_MQTT_IO_DRIVER=swoole`，旧驱动选项随内部通信迁移一并删除。设备 10000、服务 100、物理连接 10100 及会话 20000 是配置预算，尚不是目标负载通过证明。通知和导出各自配置 Redis，缓存开启与否不替代这两个后台角色的依赖。
