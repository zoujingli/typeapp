...
| Windows x64 | Swoole SDK 构建与加载、PHPUnit；独立 ORM 的三库 PHP、AOT 与移除源码运行，包含上下文、双进程乐观锁与原子更新；PostgreSQL 另验证物理连接复用、污染清理及故障退役 | 各库结果分别成立；MySQL/SQLite 物理复用、完整应用及最终同提交验收待完成 |
