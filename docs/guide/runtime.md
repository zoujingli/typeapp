...
TypePHP 编译后的应用仍需要管理并发工作和资源生命周期。TypeApp 以执行作用域组织请求、消息与后台任务，内置 Swoole 运行库负责进程、线程、协程、通信和事件循环。业务负责状态与资源归属。

本页说明三种执行层次的知识、使用方式和控制边界。它与 [系统架构](architecture.md)、[基础通信](communications.md) 和 [type-runtime](plugins/type-runtime.md) 共同构成运行时约定。

选择执行方式时同时确定隔离程度、资源预算和停止责任。并发、吞吐和尾延迟的测量方法见[性能与调优](performance.md)。
...
1. 复制父作用域的字符串关联信息及应用绑定值，子任务自动绑定自己的作用域；父后续修改或子作用域读取数组的修改不会改变创建时的快照。
...
- `Swoole\Process` 的管道、信号和退出码用于角色控制；在已有协程中启动独立应用命令，使用官方 `SWOOLE_HOOK_PROC` 接管的参数数组和管道，不在协程内 fork 后执行 PHP 回调。必须同时验证响应内容、退出状态和真实回收；业务结果仍需通过明确的消息或持久状态交接。
