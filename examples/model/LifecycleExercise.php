...
final class LifecycleExercise
{
    public static function verify(Connection $connection): array
    {
        ...
        self::check($document->save($connection) === 'created', '创建事件没有贯通');
        $stored = Document::query($connection)->where('title', '=', 'hello')->first();
        $id = $stored->getId();
        ...
        $observer->clear();
        self::check($document->save($connection) === 'unchanged' && $observer->events() === [], '无变更触发了写入事件');
        $cancelled = new Document(['title' => 'cancel', 'status' => 'draft'], false, $behavior);
        self::check($cancelled->save($connection) === 'cancelled' && !$cancelled->isPersisted()
            && Document::query($connection)->count() === 1, '取消创建仍然持久化');
        $observer->clear();
        self::check($document->delete($connection) && Document::query($connection)->find($id) === null, '软删除默认过滤失效或展示主键改变写入目标');
        self::check(Document::query($connection)->withTrashed()->count() === 1 && Document::query($connection)->onlyTrashed()->count() === 1, '包含或仅已删除行为错误');
        self::check($observer->events() === ['deleting', 'deleted'] && $document->getDeletedAt() !== null, '删除事件或模型标记错误');
        $version = $document->getVersion();
        self::check(!$document->delete($connection) && $document->getVersion() === $version, '重复软删除推进了版本');
        $observer->clear();
        self::check($document->restore($connection) && Document::query($connection)->count() === 1
            && $observer->events() === ['restoring', 'restored'], '恢复行为或事件错误');
        $version = $document->getVersion();
        self::check(!$document->restore($connection) && $document->getVersion() === $version, '重复恢复推进了版本');
        $other = new Document(['title' => 'other', 'status' => 'archived']);
        $other->save($connection);
        $base = Document::query($connection);
        ...
        $changed = Document::query($connection)->withBehavior($behavior)->find($id);
        self::check($changed->getTitle() === 'BULK' && $observer->events() === ['retrieved'], '读取事件或获取器失效');
        $changed->setTitle(' fail ');
        try {
            $changed->save($connection);
        } catch (RuntimeException $error) {
            self::check($error->getMessage() === '后置事件失败', '后置事件错误丢失');
        }
        self::check(Document::query($connection)->find($id)->getTitle() === 'bulk', '后置事件失败后没有回滚写入');
        $invalid = false;
        try {
            $changed->getTitle();
        ...
        $observer->clear();
        $fresh = Document::query($connection)->withBehavior($behavior)->find($id);
        $observer->clear();
        self::check($fresh->forceDelete($connection) && Document::query($connection)->withTrashed()->find($id) === null
            && $observer->events() === ['forceDeleting', 'forceDeleted'], '强制删除或事件不正确');
        $other->forceDelete($connection);
