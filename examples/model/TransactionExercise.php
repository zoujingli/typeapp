...
final class TransactionExercise
{
    public static function verify(Connection $connection): array
    {
        ...
        self::check(User::query()->count() === $before + 1, '内层回滚范围不正确');
        try {
            $connection->transaction(static function (Connection $transaction) use (&$state): void {
                $state['partial'] = User::query()->select(['name'])->find($state['kept']->getId());
        ...
        self::check(User::query()->count() === $before + 1
            && User::query()->find($state['kept']->getId())->getName() === '外层保留', '外层没有回滚内层成功或部分字段写入');
        $rejected = false;
        try {
            $state['merged']->save();
        ...
        $state['kept']->delete();
        self::check($connection->transactionDepth() === 0 && User::query()->count() === $before, '事务结束后仍有登记或残留记录');
