...
function operationsUnknown(Connection $connection, UserOperations $operations, TypedCache $cache): void
{
    operationsAssert($operations->cached($connection, $cache, 1) === 'before', '未知提交前缓存没有准备好');
    $outcome = '';
    try {
        $operations->rename($connection, $cache, 1, 'after');
    ...
    $cached = $operations->cached($connection, $cache, 1);
...
{
    $database = new Database(Drivers::create((string) ($argv[1] ?? 'sqlite')));
    ...
    try {
        ...
        if (($argv[2] ?? '') === 'unknown') {
            operationsUnknown($connection, $operations, $cache);
            return;
        }
        $operations->initialize($connection);
        $operations->create($connection, 1, '中文事务');
        operationsAssert($operations->read($connection, 1) === '中文事务', '事务成功后未读取到业务结果');
        $failed = false;
        try {
            $operations->create($connection, 2, '应回滚', true);
        } catch (RuntimeException $error) {
            $failed = $error->getMessage() === '预期业务失败';
        }
        operationsAssert($failed && $operations->read($connection, 2) === null, '事务异常没有回滚或改变原始异常');
        try {
            $service->create($connection, 3, '普通调用', true);
        } catch (RuntimeException) {
        }
        operationsAssert($operations->read($connection, 3) === '普通调用', '原业务方法不应被隐式拦截');
        operationsAssert($operations->cached($connection, $cache, 1) === '中文事务'
            && $operations->cached($connection, $cache, 1) === '中文事务' && $operations->loads() === 1, '生成操作没有复用缓存命中');
        operationsAssert($operations->cached($connection, $cache, 9) === null && $operations->cached($connection, $cache, 9) === null
            && $operations->loads() === 2, '缓存 null 被当作未命中');
        $operations->shortLived($connection, $cache, 1);
        usleep(60000);
        $operations->shortLived($connection, $cache, 1);
        ...
        try {
            $operations->rename($connection, $cache, 1, '失败修改', true);
        } catch (RuntimeException) {
        }
        operationsAssert($operations->cached($connection, $cache, 1) === '中文事务' && $operations->loads() === 6, '业务回滚时错误清理了缓存');
        $outerFailed = false;
        try {
            $connection->transaction(static function (Connection $outer) use ($operations, $cache): void {
                $operations->rename($outer, $cache, 1, '嵌套回滚');
                operationsAssert($operations->cached($outer, $cache, 1) === '中文事务', '内层提交提前清理外层事务缓存');
                throw new RuntimeException('预期外层回滚');
            });
        } catch (RuntimeException $outerError) {
            $outerFailed = $outerError->getMessage() === '预期外层回滚';
        }
        operationsAssert($outerFailed && $operations->read($connection, 1) === '中文事务'
            && $operations->cached($connection, $cache, 1) === '中文事务' && $operations->loads() === 6, '外层回滚没有丢弃缓存失效回调');
        $connection->transaction(static function (Connection $outer) use ($operations, $cache): void {
            operationsAssert($operations->rename($outer, $cache, 1, '提交后更新') === '提交后更新', '事务方法返回值错误');
            operationsAssert($operations->cached($outer, $cache, 1) === '中文事务', '缓存未等待最外层提交');
        });
        operationsAssert($operations->cached($connection, $cache, 1) === '提交后更新' && $operations->loads() === 7, '最外层提交后未失效缓存');
        try {
            $operations->evict($cache, 1, true);
        } catch (RuntimeException) {
        }
        $operations->cached($connection, $cache, 1);
        ...
        $operations->clearAfterWrite($connection, $cache, 1, ['name' => '按代次清理']);
        operationsAssert($operations->cached($connection, $cache, 1) === '按代次清理' && $operations->loads() === 9, '事务成功后没有清理当前命名空间');
        ...
        try {
            $operations->rename($connection, $cache, 1, '已提交但失效失败');
        ...
        $redis = $redisManager->connection($scope, 'default', Purpose::SCRIPT);
        $cache = new TypedCache(new NamespaceStore($redis, $cacheApplication, 'test', 'v1'), JsonCodec::data());
        operationsAssert($committedFailure && $operations->read($connection, 1) === '已提交但失效失败'
            && $operations->cached($connection, $cache, 1) === '按代次清理', '失效回调失败没有保留已提交事实和旧缓存');
