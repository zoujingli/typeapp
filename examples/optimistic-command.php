    ...
    \Type\Runtime\CoroutineRuntime::run(static function () use ($argv): void {
        ...
        try {
            $scope->run(static function (ExecutionScope $current) use ($database, &$scope, $argv, $driver): void {
                ...
                $first = Counter::query()->select(['value'])->find(1);
                $stale = Counter::query()->find(1);
                optimisticExpect($first->getVersion() === 1, '部分查询没有保留版本');
                $first->setValue(1);
                optimisticExpect($first->save() === 'updated' && $first->getVersion() === 2, '更新没有原子推进版本');
                $stale->setValue(2);
                $conflict = false;
                try {
                    $stale->save();
                ...
                optimisticExpect($conflict && Counter::query()->find(1)->getValue() === 1, '过期版本覆盖了最新数据');
                $invalid = false;
                try {
                    $stale->getVersion();
                } catch (ModelException $error) {
                ...
                optimisticExpect($first->save() === 'unchanged' && $first->getVersion() === 2, '无变更推进了版本');
                $missing = Counter::query()->find(2);
                ...
                try {
                    $connection->transaction(static function (Connection $transaction) use (&$rolledBack): void {
                        $rolledBack = Counter::query()->find(1);
                ...
                $staleDelete = Counter::query()->find(1);
                $current->setValue(3);
                $current->save();
                $conflict = false;
                try {
                    $staleDelete->forceDelete();
                } catch (ModelException $error) {
                    $conflict = $error->errorCode() === 'optimistic_conflict';
                }
                optimisticExpect($conflict && Counter::query()->find(1)->getValue() === 3, '过期版本删除了最新对象');
                Counter::query()->find(1)->forceDelete();
                $protected = false;
                $guarded = Counter::query()->find(3);
                try {
