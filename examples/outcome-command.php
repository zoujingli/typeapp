...
function main(int $argc, array $argv): void
{
    ...
    try {
        if ($fault) {
            $rwManager = new DatabaseManager(['primary' => Drivers::create('mysql'), 'replica' => new MysqlDriver(
            ...
            $session = new ReadWriteSession($rwManager, $scope);
            $calls = 0;
            $callbacks = 0;
            $model = null;
            $outcome = '';
            try {
                $session->transaction(static function (Connection $transaction) use (&$calls, &$callbacks, &$model): void {
                    $calls++;
                    $model = new User(['name' => '故障写入', 'age' => 20, 'active' => true, 'secret' => '内部']);
                    $model->save($transaction);
                    $transaction->afterCommit(static function () use (&$callbacks): void {
                        $callbacks++;
                    });
                });
            ...
            if ($outcome === TransactionOutcome::UNKNOWN) {
                ...
                outcomeExpect($blocked && $session->outcome() === TransactionOutcome::UNKNOWN, '提交未知被主库粘滞掩盖');
            }
