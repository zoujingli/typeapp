...
function main(int $argc, array $argv): void
{
    ...
    $database = new Database(Drivers::create($driver), 1, 1);
    ...
    try {
        ...
        $relation = Relation::hasMany(static fn (Connection $connection): ModelQuery => Article::query($connection)->select(['title']), 'user_id', 'id', 3);
        $modelQuery = User::query($connection)->select(['name'])->with('articles', $relation);
        ...
        $owner = Relation::belongsTo(static fn (Connection $connection): ModelQuery => User::query($connection)->select(['name']), 'user_id');
        $nested = Relation::hasMany(static fn (Connection $connection): ModelQuery => Article::query($connection)->with('owner', $owner), 'user_id');
        paginationExpect(paginationReject(static fn () => User::query($connection)->with('articles', $nested)->cursorPaginate(4, null, 12)), '嵌套预加载没有共享行数预算');
        ...
        $tags = Relation::belongsToMany(static fn (Connection $connection): ModelQuery => \TypeApp\ModelExample\Tag::query($connection), 'type_pagination_user_tags', 'user_id', 'tag_id');
        $tagged = User::query($connection)->with('tags', $tags);
        ...
        paginationExpect((int) $stream->next()['id'] === 1 && $database->statistics()['leased'] === 1, '流没有持有连接租约');
        ...
        $modelRows = User::query($connection)->select(['name'])->with('articles', $relation)->chunk(250, static function (array $models): void {
        ...
        $pending->next();
        $scope->close();
        paginationExpect($pending->closed() && $database->statistics()['leased'] === 0, '作用域结束没有清理未显式关闭的流');
        $pending->close();
        $scope = new ExecutionScope();
        $reopened = $database->connect($scope);
        paginationExpect((int) $reopened->query('SELECT 7 AS value')[0]['value'] === 7, '流清理后连接池不能重新借用');
        $reopened->close();
        echo json_encode(['driver' => $driver, 'rows' => $count, 'sum' => $sum, 'peak_growth' => $growth, 'model_peak_growth' => $modelGrowth, 'leased' => $database->statistics()['leased']], JSON_THROW_ON_ERROR) . PHP_EOL;
    } finally {
        $scope->close();
        $database->close();
    }
