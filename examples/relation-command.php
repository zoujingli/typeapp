...
{
    $database = new Database(Drivers::create((string) ($argv[1] ?? 'sqlite')), 1, 0);
    ...
    try {
        ...
        $articles = Relation::hasMany(static fn (Connection $connection): ModelQuery => Article::query($connection)->select(['title']), 'user_id', 'id', 2);
        $profiles = Relation::hasOne(static fn (Connection $connection): ModelQuery => Profile::query($connection), 'user_id', 'id', 2);
        $base = User::query($connection)->select(['name']);
        ...
        $owner = Relation::belongsTo(static fn (Connection $connection): ModelQuery => User::query($connection)->select(['name']), 'user_id', 'id', 2);
        $posts = Article::query($connection)->select(['title'])->with('owner', $owner)->orderBy('id')->get();
        ...
        $rejected = false;
        try {
            $base->with('articles', Relation::hasMany(static fn (Connection $connection): ModelQuery => Article::query($connection)->limit(1), 'user_id'))->get();
        } catch (ModelException $error) {
