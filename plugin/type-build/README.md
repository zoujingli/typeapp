...
构建声明可通过 `threads` 登记已编译业务入口。构建器核对受控 Swoole/PHPX ABI 2、源码摘要与 fiber 通知配置，并在正常模块启动阶段发布应用符号。线程应用按编译统计和运行声明选择官方 Swoole 开关，把该模块链进主程序；编译探针仍加载超集共享模块，发布配置不再 `extension=swoole`，也不再复制 `swoole.so`。线程内协程作用域、初始化失败和清理边界见[已编译业务线程](https://github.com/zoujingli/typeapp/blob/main/docs/development/compiled-business-threads.md)，实际平台结果按该页记录。
