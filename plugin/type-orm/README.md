...
回调签名由调用入口固定，TypePHP 不会忽略多余实参；未使用的参数也应保留。事务体为 `Closure(Connection): mixed`，`afterCommit()` 为零参数回调；条件分组为 `Closure(Conditions): Conditions`；关系工厂为 `Closure(Connection): ModelQuery`。`ModelQuery::scope()` 接收 `Closure(ModelQuery): ModelQuery`，`search()` 的每个搜索器接收 `Closure(ModelQuery, mixed): ModelQuery`；模型 getter/setter 只接收字段值。普通和模型 `chunk()` 分别传一个行数组或模型数组，`RowStream::each()` 传一个当前行数组，仅返回 `false` 提前结束，也可以使用 `void` 消费者。
...
下面是应用服务中的完整声明式函数，纳入应用源码后由应用入口调用；不是有独立 main 的数据库程序。连接、表迁移与资源所有权由调用者提供，完整可运行入口见各驱动组件。
...
上例是可纳入应用 AOT 的业务函数，需要调用方预先准备 `users` 表并从当前 Scope 取得连接；三库的完整程序见驱动组件。模型直接继承 `Type\Orm\Model`，以 PHP 类型属性和 `Table/Column` Attribute 声明字段；构建生成同名业务类的水合工厂与属性钩子，保留业务方法。业务通过 `$user->name` 读写字段，不再声明 JSON 或继承生成基类。开发入口先加载本代模型；生产编译同一转换结果。`Outbox\Store/Relay/Publisher` 将持久意图和外部投递分开，仍需稳定操作 ID 与目标端幂等，不保证跨系统事务。可选 `#[Transactional(connection: 'connection')]` 只有通过 type-build 的显式生成包装才执行，直接调用原方法不发生拦截。
