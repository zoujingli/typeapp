...
{
    expect($old['host'] === $new['host'] && $old['engine'] === $new['engine'] && $old['driver'] === $new['driver']
        && $old['sampling_protocol'] === 2 && $new['sampling_protocol'] === 2, '基准平台、引擎、驱动或采样协议不可比');
    expect(count($old['repetitions']) >= 3 && count($new['repetitions']) === count($old['repetitions']), '基准须有相同且至少三个独立轮次');
    $results = [];
    foreach (['short-json', 'crud', 'slow-database', 'file-stream'] as $workload) {
...
expect($report['status'] === 'measured-not-compared' && count($report['runs']) === 12, '须先完成两引擎、三库、两个版本的全部测量');
$groups = [];
$artifacts = [];
foreach ($report['runs'] as $run) {
    expect(in_array($run['engine'], ['stream', 'swoole'], true) && in_array($run['driver'], ['sqlite', 'mysql', 'pgsql'], true), '无效的引擎或数据库组合');
    ...
    expect($run['measurement']['status'] === 'passed' && $run['measurement']['engine'] === $run['engine']
        && $run['measurement']['driver'] === $run['driver'], '测量结果没有通过或组合身份不符');
    $key = $run['engine'] . '/' . $run['driver'];
    expect(in_array($run['version'], ['old', 'new'], true) && !isset($groups[$key][$run['version']]), '重复或无效的版本测量');
    $identity = [$run['measurement']['artifact_sha256'], $run['measurement']['stream_artifact_sha256']];
    expect(!isset($artifacts[$run['version']]) || $artifacts[$run['version']] === $identity, '同一版本的跨引擎/数据库测量更换了产物');
    $artifacts[$run['version']] = $identity;
    $groups[$key][$run['version']] = $run['measurement'];
}
expect(count($groups) === 6, '成对测量没有覆盖六个引擎/数据库组合');
