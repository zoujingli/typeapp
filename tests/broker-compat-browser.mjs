...
/**
 * 真实浏览器核对独立节点页展示升级契约与危险回退维护步骤，以及租户/平台只读查询。
 */

const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const args = process.argv.slice(2);
if (args.length !== 5) throw new Error('compat_browser_arguments_invalid');
...
if (!base.startsWith(`${build}${sep}`) || !statSync(base).isDirectory() || !statSync(resolve(dist, 'index.html')).isFile()
  ...
  throw new Error('compat_browser_arguments_invalid');
}
const accounts = {
...
if (!Object.values(accounts).every(account => typeof account.password === 'string' && account.password.length >= 12 && account.password.length <= 72)
  ...
  throw new Error('compat_browser_fixture_invalid');
}
...
const onSignal = () => stop.abort(new Error('compat_browser_interrupted'));
process.on('SIGTERM', onSignal); process.on('SIGINT', onSignal);
const timer = setTimeout(() => stop.abort(new Error('compat_browser_deadline')), 180000);
...
const screenshot = async name => {
  const file = `browser-compat-${name}.png`;
...
try {
  ...
  await bounded((async () => {
    ...
    await expect(page.getByText('危险回退已阻止', { exact: true })).toBeVisible();
    await expect(page.getByText('不能删除 broker_debug_occupancy、broker_access_crl_serials、broker_access_platform_serials 或 broker_compatibility 来假装兼容。')).toBeVisible();
    await screenshot('standalone-compat');
    report.cases.push('real-standalone-nodes-show-blocked-rollback');
    await login('tenant');
    ...
    if (typeof tenantToken !== 'string' || tenantToken.length < 32) throw new Error('compat_browser_tenant_token_missing');
    const tenantCompat = await page.request.get(`${api}/customer/tenants/${tenantId}/broker/compat`, {
      headers: { Authorization: `Bearer ${tenantToken}`, 'X-Tenant-Id': tenantId },
    });
    expect(tenantCompat.status()).toBe(200);
    const tenantBody = await tenantCompat.json();
    expect(tenantBody.rollback_allowed).toBe(false);
    expect(tenantBody.runtime_epoch).toBe(19);
    const tenantWrite = await page.request.post(`${api}/customer/tenants/${tenantId}/broker/compat`, {
    ...
    report.cases.push('real-tenant-compat-read-only');
    ...
    if (typeof platformToken !== 'string' || platformToken.length < 32) throw new Error('compat_browser_platform_token_missing');
    const platformCompat = await page.request.get(`${api}/admin/broker/compat`, {
      headers: { Authorization: `Bearer ${platformToken}` },
    });
    expect(platformCompat.status()).toBe(200);
    const platformWrite = await page.request.post(`${api}/admin/broker/compat`, {
    ...
    await screenshot('platform-compat');
    report.cases.push('real-platform-compat-read-without-write');
  })());
  if (report.errors.length) throw new Error('compat_browser_page_error');
  report.status = 'passed';
} catch (error) {
  failed = error; report.status = 'failed'; report.failure = safeError(error);
  try { await screenshot('failure'); } catch {}
} finally {
  stop.abort(new Error('compat_browser_finished'));
  ...
  writeFileSync(resolve(base, 'browser-compat-report.json'), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
}
if (report.status !== 'passed') {
  process.stderr.write('broker_compat_browser_failed; inspect browser-compat-report.json\n');
  process.exitCode = 1;
