...
/**
 * 真实浏览器核对允许 Origin 的 WSS 调试往返、管理断开关闭会话，以及拒绝 Origin 的 1008。
 */

const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const args = process.argv.slice(2);
if (args.length !== 6) throw new Error('interop_browser_arguments_invalid');
...
const deniedUrl = new URL(args[5]);
...
if (!base.startsWith(`${build}${sep}`) || !statSync(base).isDirectory() || !statSync(resolve(dist, 'index.html')).isFile()
  || [...origins, previewUrl, deniedUrl].some(origin => origin.protocol !== 'http:' || !['127.0.0.1', '[::1]'].includes(origin.hostname)
    || origin.username || origin.password || origin.pathname !== '/' || origin.search || origin.hash)) {
  throw new Error('interop_browser_arguments_invalid');
}
const accounts = {
...
const wssPort = Number(process.env.INTEROP_WSS_PORT || 0);
if (!Object.values(accounts).every(account => typeof account.password === 'string' && account.password.length >= 12 && account.password.length <= 72)
  || !/^[a-f0-9]{32}$/.test(tenantId) || wssPort < 1) {
  throw new Error('interop_browser_fixture_invalid');
}
...
const report = { status: 'running', fixture: relative(root, base), dist: relative(root, dist), cases: [], errors: [], screenshots: [],
  cleanup: { context: false, browser: false, preview: false, denied: false } };
const previewOwner = brokerPreview(dist, origins[1], origins[0]);
const preview = previewOwner.server;
const deniedSockets = new Set();
const denied = createHttpServer((_incoming, outgoing) => {
  outgoing.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' });
  outgoing.end('<!doctype html><title>denied</title>');
});
denied.on('connection', socket => { deniedSockets.add(socket); socket.on('close', () => deniedSockets.delete(socket)); });
const stop = new AbortController();
const onSignal = () => stop.abort(new Error('interop_browser_interrupted'));
process.on('SIGTERM', onSignal); process.on('SIGINT', onSignal);
const timer = setTimeout(() => stop.abort(new Error('interop_browser_deadline')), 180000);
...
const screenshot = async name => {
  const file = `browser-interop-${name}.png`;
...
try {
  ...
  await bounded(new Promise((resolve, reject) => { denied.once('error', reject); denied.listen(Number(deniedUrl.port), deniedUrl.hostname, resolve); }));
  report.preview = previewUrl.origin;
  report.denied = deniedUrl.origin;
  ...
  await bounded((async () => {
    ...
    const clientId = (await page.locator('.debug-meta dd').nth(1).innerText()).trim();
    if (!clientId || clientId === '—') throw new Error('interop_browser_client_id_missing');
    await screenshot('standalone-wss-roundtrip');
    report.cases.push('real-standalone-wss-debug-roundtrip');
    ...
    await expect(debugPage.getByText('已中断')).toBeVisible({ timeout: 15000 });
    await screenshot('standalone-admin-disconnect');
    report.cases.push('real-standalone-admin-disconnect-closes-wss');
    await page.close();
    page = debugPage;
    const deniedPage = await context.newPage();
    await deniedPage.goto(deniedUrl.origin);
    const closeCode = await deniedPage.evaluate(async port => await new Promise(resolve => {
      const socket = new WebSocket(`wss://127.0.0.1:${port}/mqtt`, 'mqtt');
      const timer = setTimeout(() => { socket.close(); resolve(0); }, 8000);
      socket.addEventListener('close', event => { clearTimeout(timer); resolve(event.code); });
      socket.addEventListener('error', () => {});
    }), wssPort);
    expect(closeCode).toBe(1008);
    await deniedPage.close();
    report.cases.push('real-denied-origin-wss-closed-1008');
    await login('tenant');
    ...
    report.cases.push('real-tenant-resources-read-only');
    ...
    report.cases.push('real-platform-resources-read-without-write');
  })());
  if (report.errors.length) throw new Error('interop_browser_page_error');
  report.status = 'passed';
} catch (error) {
  failed = error; report.status = 'failed'; report.failure = safeError(error);
  try { await screenshot('failure'); } catch {}
} finally {
  stop.abort(new Error('interop_browser_finished'));
  ...
  writeFileSync(resolve(base, 'browser-interop-report.json'), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
}
if (report.status !== 'passed') {
  process.stderr.write('broker_interop_browser_failed; inspect browser-interop-report.json\n');
  process.exitCode = 1;
