...
/**
 * 真实浏览器核对独立端配额预览确认发布、租户只读及平台隐藏页停止轮询。
 * 参数为隔离目录、前端 dist、独立管理源与双端预览源；口令只从进程环境读取。
 */

const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const args = process.argv.slice(2);
if (args.length !== 4) throw new Error('quota_browser_arguments_invalid');
...
if (!base.startsWith(`${build}${sep}`) || !statSync(base).isDirectory() || !statSync(resolve(dist, 'index.html')).isFile()
  ...
  throw new Error('quota_browser_arguments_invalid');
}
const accounts = {
...
if (!Object.values(accounts).every(account => typeof account.password === 'string' && account.password.length >= 12 && account.password.length <= 72)
  ...
  throw new Error('quota_browser_fixture_invalid');
}
...
const onSignal = () => stop.abort(new Error('quota_browser_interrupted'));
process.on('SIGTERM', onSignal); process.on('SIGINT', onSignal);
const timer = setTimeout(() => stop.abort(new Error('quota_browser_deadline')), 150000);
...
const screenshot = async name => {
  const file = `browser-quota-${name}.png`;
...
const openQuotas = async path => {
  const list = await actionResponse(response => response.request().method() === 'GET' && new URL(response.url()).pathname.endsWith('/broker/quotas'),
    async () => {
      await page.evaluate(path => { location.hash = path; }, path);
      await page.waitForURL(`**/#${path}`);
    });
  expect(list.status()).toBe(200);
  await expect(page.getByRole('heading', { name: 'Broker 配额', exact: true })).toBeVisible();
...
try {
  ...
  await bounded((async () => {
    ...
    await openQuotas('/broker/quotas');
    const service = page.getByRole('spinbutton', { name: '服务连接', exact: true });
    await expect(service).toBeVisible();
    const current = Number(await service.inputValue());
    await service.fill(String(Math.max(0, current - 1)));
    const previewed = await actionResponse(response => response.request().method() === 'POST' && new URL(response.url()).pathname.endsWith('/broker/quotas/preview'),
      () => page.getByRole('button', { name: '预览调整', exact: true }).click());
    expect(previewed.status()).toBe(200);
    await expect(page.getByText('确认按此额度拒绝新的超额占用', { exact: false })).toBeVisible();
    await page.getByText('确认按此额度拒绝新的超额占用', { exact: false }).click();
    const submitted = await actionResponse(response => response.request().method() === 'POST' && new URL(response.url()).pathname.endsWith('/broker/quotas') && !new URL(response.url()).pathname.endsWith('/preview'),
      () => page.getByRole('button', { name: '发布额度', exact: true }).click());
    expect(submitted.status()).toBe(200);
    await screenshot('standalone-quota-confirm');
    report.cases.push('real-standalone-quota-preview-confirm');
    await login('tenant');
    ...
    await page.waitForURL('**/#/profile');
    await openQuotas('/broker-quotas');
    await expect(page.getByRole('button', { name: '发布额度', exact: true })).toHaveCount(0);
    await screenshot('tenant-quotas');
    report.cases.push('real-tenant-quotas-read-only');
    await login('platform');
    await page.waitForURL('**/#/admin/profile');
    await openQuotas('/admin/broker-quotas');
    await screenshot('platform-quotas');
    let hidden = 0;
    const count = request => {
      const path = new URL(request.url()).pathname;
      if (request.method() === 'GET' && path.endsWith('/broker/quotas')) hidden++;
    };
    ...
    report.cases.push('real-platform-quotas-hidden-page-stops-polling');
  })());
  if (report.errors.length) throw new Error('quota_browser_page_error');
  report.status = 'passed';
} catch (error) {
  failed = error; report.status = 'failed'; report.failure = safeError(error);
  try { await screenshot('failure'); } catch {}
} finally {
  stop.abort(new Error('quota_browser_finished'));
  ...
  writeFileSync(resolve(base, 'browser-quota-report.json'), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
}
if (report.status !== 'passed') {
  process.stderr.write('broker_quota_browser_failed; inspect browser-quota-report.json\n');
  process.exitCode = 1;
