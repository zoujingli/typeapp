...
/**
 * 真实浏览器核对独立节点页恢复进度、授权页核对标记，以及租户/平台只读恢复查询。
 */

const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const args = process.argv.slice(2);
if (args.length !== 5) throw new Error('recovery_browser_arguments_invalid');
...
if (!base.startsWith(`${build}${sep}`) || !statSync(base).isDirectory() || !statSync(resolve(dist, 'index.html')).isFile()
  ...
  throw new Error('recovery_browser_arguments_invalid');
}
const accounts = {
...
if (!Object.values(accounts).every(account => typeof account.password === 'string' && account.password.length >= 12 && account.password.length <= 72)
  ...
  throw new Error('recovery_browser_fixture_invalid');
}
...
const onSignal = () => stop.abort(new Error('recovery_browser_interrupted'));
process.on('SIGTERM', onSignal); process.on('SIGINT', onSignal);
const timer = setTimeout(() => stop.abort(new Error('recovery_browser_deadline')), 180000);
...
const screenshot = async name => {
  const file = `browser-recovery-${name}.png`;
...
try {
  ...
  await bounded((async () => {
    ...
    const nodes = await actionResponse(response => response.request().method() === 'GET' && new URL(response.url()).pathname.endsWith('/broker/nodes'),
      async () => { await page.waitForURL('**/#/broker/nodes'); });
    expect(nodes.status()).toBe(200);
    await expect(page.getByRole('heading', { name: 'Broker 节点', exact: true })).toBeVisible();
    await expect(page.getByText('恢复核对已完成', { exact: true })).toBeVisible();
    await screenshot('standalone-recovery-nodes');
    report.cases.push('real-standalone-nodes-show-recovery-ready');
    await page.evaluate(() => { location.hash = '/broker/access'; });
    const access = await actionResponse(response => response.request().method() === 'GET' && new URL(response.url()).pathname.endsWith('/broker/access/principals'),
      async () => { await page.waitForURL('**/#/broker/access'); });
    ...
    await screenshot('standalone-recovery-access');
    report.cases.push('real-standalone-access-shows-unverified');
    await page.evaluate(() => { location.hash = '/broker/debug'; });
    const debug = await actionResponse(response => response.request().method() === 'GET' && new URL(response.url()).pathname.endsWith('/broker/debug'),
      async () => { await page.waitForURL('**/#/broker/debug'); });
    expect(debug.status()).toBe(200);
    await expect(page.getByRole('heading', { name: 'Broker 调试订阅', exact: true })).toBeVisible();
    await expect(page.getByText('未核对的调试凭据', { exact: false })).toBeVisible();
    await screenshot('standalone-recovery-debug');
    report.cases.push('real-standalone-debug-shows-isolated');
    await login('tenant');
    ...
    await page.waitForURL('**/#/profile');
    const api = origins[1].origin;
    const tenantToken = await page.evaluate(() => sessionStorage.getItem('typeapp.customer.token'));
    if (typeof tenantToken !== 'string' || tenantToken.length < 32) throw new Error('recovery_browser_tenant_token_missing');
    const tenantRecovery = await page.request.get(`${api}/customer/tenants/${tenantId}/broker/recovery`, {
      headers: { Authorization: `Bearer ${tenantToken}`, 'X-Tenant-Id': tenantId },
    });
    expect(tenantRecovery.status()).toBe(200);
    const tenantBody = await tenantRecovery.json();
    expect(tenantBody.host).toBe('app');
    const tenantWrite = await page.request.post(`${api}/customer/tenants/${tenantId}/broker/recovery`, {
      headers: { Authorization: `Bearer ${tenantToken}`, 'X-Tenant-Id': tenantId, 'Content-Type': 'application/json' },
      data: {},
    });
    expect([403, 404, 405]).toContain(tenantWrite.status());
    report.cases.push('real-tenant-recovery-read-only');
    await login('platform');
    await page.waitForURL('**/#/admin/profile');
    const platformToken = await page.evaluate(() => sessionStorage.getItem('typeapp.admin.token'));
    if (typeof platformToken !== 'string' || platformToken.length < 32) throw new Error('recovery_browser_platform_token_missing');
    const platformRecovery = await page.request.get(`${api}/admin/broker/recovery`, {
      headers: { Authorization: `Bearer ${platformToken}` },
    });
    expect(platformRecovery.status()).toBe(200);
    const platformWrite = await page.request.post(`${api}/admin/broker/recovery`, {
      headers: { Authorization: `Bearer ${platformToken}`, 'Content-Type': 'application/json' },
      data: {},
    });
    expect([403, 404, 405]).toContain(platformWrite.status());
    await screenshot('platform-recovery');
    report.cases.push('real-platform-recovery-read-without-write');
  })());
  if (report.errors.length) throw new Error('recovery_browser_page_error');
  report.status = 'passed';
} catch (error) {
  failed = error; report.status = 'failed'; report.failure = safeError(error);
  try { await screenshot('failure'); } catch {}
} finally {
  stop.abort(new Error('recovery_browser_finished'));
  ...
  writeFileSync(resolve(base, 'browser-recovery-report.json'), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
}
if (report.status !== 'passed') {
  process.stderr.write('broker_recovery_browser_failed; inspect browser-recovery-report.json\n');
  process.exitCode = 1;
