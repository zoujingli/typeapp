...
/**
 * 真实浏览器核对独立端保留预览确认清除、租户无持久存储动作及平台隐藏页停止轮询。
 * 参数为隔离目录、前端 dist、独立管理源与双端预览源；口令只从进程环境读取。
 */

const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const args = process.argv.slice(2);
if (args.length !== 4) throw new Error('clear_browser_arguments_invalid');
...
if (!base.startsWith(`${build}${sep}`) || !statSync(base).isDirectory() || !statSync(resolve(dist, 'index.html')).isFile()
  ...
  throw new Error('clear_browser_arguments_invalid');
}
const accounts = {
...
const topic = process.env.BROKER_BROWSER_TOPIC || '';
if (!Object.values(accounts).every(account => typeof account.password === 'string' && account.password.length >= 12 && account.password.length <= 72)
  || !/^[a-f0-9]{32}$/.test(tenantId) || topic === '') {
  throw new Error('clear_browser_fixture_invalid');
}
...
const onSignal = () => stop.abort(new Error('clear_browser_interrupted'));
process.on('SIGTERM', onSignal); process.on('SIGINT', onSignal);
const timer = setTimeout(() => stop.abort(new Error('clear_browser_deadline')), 150000);
...
const screenshot = async name => {
  const file = `browser-retain-clear-${name}.png`;
...
const openRetained = async path => {
  const list = await actionResponse(response => response.request().method() === 'GET' && new URL(response.url()).pathname.endsWith('/broker/resources/retained'),
    async () => {
      await openResources(path);
      await page.getByLabel('资源类型').click();
      await page.getByTitle('保留消息', { exact: true }).click();
    });
...
try {
  ...
  await bounded((async () => {
    ...
    const list = await openRetained('/broker/resources');
    const data = await list.json();
    expect(data.items.some(item => item.topic === topic)).toBe(true);
    const target = rows().filter({ hasText: topic }).first();
    await expect(target.getByRole('button', { name: '清除', exact: true })).toBeVisible();
    const previewed = await actionResponse(response => response.request().method() === 'GET' && new URL(response.url()).pathname.endsWith('/clearance-preview'),
      () => target.getByRole('button', { name: '清除', exact: true }).click());
    expect(previewed.status()).toBe(200);
    await expect(page.getByText('将取消该保留原件未来重放', { exact: false })).toBeVisible();
    const submitted = await actionResponse(response => response.request().method() === 'POST' && new URL(response.url()).pathname.endsWith('/clear'),
      () => page.getByRole('button', { name: '清除保留', exact: true }).click());
    ...
    await expect(page.locator('.ant-spin-spinning')).toHaveCount(0);
    await screenshot('standalone-retain-clear-confirm');
    report.cases.push('real-standalone-retain-clear-confirm');
    await login('tenant');
    ...
    await page.waitForURL('**/#/profile');
    await openResources('/broker-resources');
    await page.getByLabel('资源类型').click();
    await page.getByTitle('保留消息', { exact: true }).click();
    await expect(page.getByRole('heading', { name: 'Broker 资源', exact: true })).toBeVisible();
    await expect(page.getByRole('button', { name: '清除', exact: true })).toHaveCount(0);
    await screenshot('tenant-retained');
    report.cases.push('real-tenant-retained-page-without-store');
    await login('platform');
    await page.waitForURL('**/#/admin/profile');
    await openResources('/admin/broker');
    await screenshot('platform-resources');
    let hidden = 0;
    const count = request => {
      const path = new URL(request.url()).pathname;
      if (request.method() === 'GET' && (path.endsWith('/broker/resources/retained') || path.endsWith('/broker/resources/connections') || path.includes('/broker/operations/'))) hidden++;
    };
  ...
  if (report.errors.length) throw new Error('clear_browser_page_error');
  report.status = 'passed';
} catch (error) {
  failed = error; report.status = 'failed'; report.failure = safeError(error);
  try { await screenshot('failure'); } catch {}
} finally {
  stop.abort(new Error('clear_browser_finished'));
  ...
  writeFileSync(resolve(base, 'browser-retain-clear-report.json'), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
}
if (report.status !== 'passed') {
  process.stderr.write('broker_retain_clear_browser_failed; inspect browser-retain-clear-report.json\n');
  process.exitCode = 1;
