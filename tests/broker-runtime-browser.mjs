...
/**
 * 真实浏览器核对独立端运行配置预览确认保存、租户只读及平台隐藏页停止轮询。
 * 参数为隔离目录、前端 dist、独立管理源与双端预览源；口令只从进程环境读取。
 */

const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const args = process.argv.slice(2);
if (args.length !== 4) throw new Error('runtime_browser_arguments_invalid');
...
if (!base.startsWith(`${build}${sep}`) || !statSync(base).isDirectory() || !statSync(resolve(dist, 'index.html')).isFile()
  ...
  throw new Error('runtime_browser_arguments_invalid');
}
const accounts = {
...
if (!Object.values(accounts).every(account => typeof account.password === 'string' && account.password.length >= 12 && account.password.length <= 72)
  ...
  throw new Error('runtime_browser_fixture_invalid');
}
...
const onSignal = () => stop.abort(new Error('runtime_browser_interrupted'));
process.on('SIGTERM', onSignal); process.on('SIGINT', onSignal);
const timer = setTimeout(() => stop.abort(new Error('runtime_browser_deadline')), 150000);
...
const screenshot = async name => {
  const file = `browser-runtime-${name}.png`;
...
const openRuntime = async path => {
  const list = await actionResponse(response => response.request().method() === 'GET' && new URL(response.url()).pathname.endsWith('/broker/runtime'),
    async () => {
      await page.evaluate(path => { location.hash = path; }, path);
      await page.waitForURL(`**/#${path}`);
    });
  expect(list.status()).toBe(200);
  await expect(page.getByRole('heading', { name: 'Broker 运行配置', exact: true })).toBeVisible();
...
try {
  ...
  await bounded((async () => {
    ...
    await openRuntime('/broker/runtime');
    const port = page.getByRole('spinbutton', { name: 'TCP 端口', exact: true });
    await expect(port).toBeVisible();
    const current = Number(await port.inputValue());
    const next = current >= 65000 ? current - 17 : current + 17;
    await port.fill(String(next));
    const previewed = await actionResponse(response => response.request().method() === 'POST' && new URL(response.url()).pathname.endsWith('/broker/runtime/preview'),
      () => page.getByRole('button', { name: '预览保存', exact: true }).click());
    expect(previewed.status()).toBe(200);
    await expect(page.getByText('确认仅校验并保存，实际重启与滚动分发由运维执行', { exact: false })).toBeVisible();
    await page.getByText('确认仅校验并保存，实际重启与滚动分发由运维执行', { exact: false }).click();
    const submitted = await actionResponse(response => response.request().method() === 'POST' && new URL(response.url()).pathname.endsWith('/broker/runtime') && !new URL(response.url()).pathname.endsWith('/preview'),
      () => page.getByRole('button', { name: '保存配置', exact: true }).click());
    expect(submitted.status()).toBe(200);
    await screenshot('standalone-runtime-confirm');
    report.cases.push('real-standalone-runtime-preview-confirm');
    await login('tenant');
    ...
    await screenshot('tenant-runtime');
    report.cases.push('real-tenant-runtime-read-only');
    await login('platform');
    await page.waitForURL('**/#/admin/profile');
    await openRuntime('/admin/broker-runtime');
    await screenshot('platform-runtime');
    let hidden = 0;
    const count = request => {
      const path = new URL(request.url()).pathname;
      if (request.method() === 'GET' && path.endsWith('/broker/runtime')) hidden++;
    };
    ...
    report.cases.push('real-platform-runtime-hidden-page-stops-polling');
  })());
  if (report.errors.length) throw new Error('runtime_browser_page_error');
  report.status = 'passed';
} catch (error) {
  failed = error; report.status = 'failed'; report.failure = safeError(error);
  try { await screenshot('failure'); } catch {}
} finally {
  stop.abort(new Error('runtime_browser_finished'));
  ...
  writeFileSync(resolve(base, 'browser-runtime-report.json'), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
}
if (report.status !== 'passed') {
  process.stderr.write('broker_runtime_browser_failed; inspect browser-runtime-report.json\n');
  process.exitCode = 1;
