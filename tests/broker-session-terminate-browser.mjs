...
/**
 * 真实浏览器核对独立端会话预览确认终止、租户无持久存储动作及平台隐藏页停止轮询。
 * 参数为隔离目录、前端 dist、独立管理源与双端预览源；口令只从进程环境读取。
 */

const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const args = process.argv.slice(2);
if (args.length !== 4) throw new Error('terminate_browser_arguments_invalid');
...
if (!base.startsWith(`${build}${sep}`) || !statSync(base).isDirectory() || !statSync(resolve(dist, 'index.html')).isFile()
  ...
  throw new Error('terminate_browser_arguments_invalid');
}
const accounts = {
...
if (!Object.values(accounts).every(account => typeof account.password === 'string' && account.password.length >= 12 && account.password.length <= 72)
  ...
  throw new Error('terminate_browser_fixture_invalid');
}
...
const onSignal = () => stop.abort(new Error('terminate_browser_interrupted'));
process.on('SIGTERM', onSignal); process.on('SIGINT', onSignal);
const timer = setTimeout(() => stop.abort(new Error('terminate_browser_deadline')), 150000);
...
const screenshot = async name => {
  const file = `browser-session-terminate-${name}.png`;
...
const openSessions = async path => {
  const list = await actionResponse(response => response.request().method() === 'GET' && new URL(response.url()).pathname.endsWith('/broker/resources/sessions'),
    async () => {
...
try {
  ...
  await bounded((async () => {
    ...
    const list = await openSessions('/broker/resources');
    const data = await list.json();
    expect(data.items.some(item => item.client_id === clientId)).toBe(true);
    const target = rows().filter({ hasText: clientId }).first();
    await expect(target.getByRole('button', { name: '终止', exact: true })).toBeVisible();
    const previewed = await actionResponse(response => response.request().method() === 'GET' && new URL(response.url()).pathname.endsWith('/termination-preview'),
      () => target.getByRole('button', { name: '终止', exact: true }).click());
    expect(previewed.status()).toBe(200);
    await expect(page.getByText('将结束该持久会话并放弃未完成交付', { exact: false })).toBeVisible();
    const submitted = await actionResponse(response => response.request().method() === 'POST' && new URL(response.url()).pathname.endsWith('/terminate'),
      () => page.getByRole('button', { name: '终止会话', exact: true }).click());
    ...
    await expect(page.locator('.ant-spin-spinning')).toHaveCount(0);
    await screenshot('standalone-session-terminate-confirm');
    report.cases.push('real-standalone-session-terminate-confirm');
    await login('tenant');
    ...
    await page.getByTitle('会话', { exact: true }).click();
    await expect(page.getByRole('heading', { name: 'Broker 资源', exact: true })).toBeVisible();
    await expect(page.getByRole('button', { name: '终止', exact: true })).toHaveCount(0);
    await screenshot('tenant-sessions');
    report.cases.push('real-tenant-sessions-page-without-store');
    ...
    const count = request => {
      ...
      if (request.method() === 'GET' && (path.endsWith('/broker/resources/sessions') || path.endsWith('/broker/resources/connections') || path.includes('/broker/operations/'))) hidden++;
    };
  ...
  if (report.errors.length) throw new Error('terminate_browser_page_error');
  report.status = 'passed';
} catch (error) {
  failed = error; report.status = 'failed'; report.failure = safeError(error);
  try { await screenshot('failure'); } catch {}
} finally {
  stop.abort(new Error('terminate_browser_finished'));
  ...
  writeFileSync(resolve(base, 'browser-session-terminate-report.json'), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
}
if (report.status !== 'passed') {
  process.stderr.write('broker_session_terminate_browser_failed; inspect browser-session-terminate-report.json\n');
  process.exitCode = 1;
