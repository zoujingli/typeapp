...
{
    $original = "PendingCommit::work(\$store, \$arguments->text('store-worker', ''));";
    expect(substr_count($source, $original) === 1, '可靠发布worker装配入口变化');
    return str_replace($original, <<<'PHP'
PendingCommit::work($store, $arguments->text('store-worker', ''), function (array $request) use ($store): \Type\Mqtt\CommitResult {
            $subscriptionGate = '';
